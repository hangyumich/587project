...
//  Sokoban
//
//  Created by Luyao Yuan on 2016/3/21.
...
#define MAX_SEARCH_EFFORT 300000
...
//    		if(effortLimit > MAX_SEARCH_EFFORT) break;
    		...
    		if(!p.open.empty())
    		{
    			...
    			if(p.steps.size() >= 2)
    			{
    				...
    				{
    					auto unifyList = p.steps[i].adds(p.open[0].first, tracker);
    					if(!unifyList.empty())	
    					{
